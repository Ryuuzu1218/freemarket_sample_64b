$(function() {
  ...
  function childrenBox(buildHTML){
    let childSelectHtml = '';
    childSelectHtml = `<select class='ex__detail__category--select' id='child-form' name = 'category_id'>
                        <option value="選択してください" data-category="選択してください"></option>
                          ${buildHTML}
                        <select>`;
    $('.ex__detail__category').append(childSelectHtml);
  }
  // 孫カテゴリー用html
  function grandchildrenBox(buildHTML){
    let grandchildSelectHtml = '';
    grandChildSelectHtml = `<select class='ex__detail__category--select' id='grandchild-form' name = 'category_id'>
                              <option value="選択してください" data-category="選択してください"></option>
                                ${buildHTML}
                              <select>`;
    $('.ex__detail__category').append(grandchildSelectHtml);
  }
